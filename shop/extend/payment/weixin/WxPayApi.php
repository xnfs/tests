...
class WxPayApi
{
	...
	public function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		//关联参数
		if($inputObj->GetTrade_type() == "JSAPI" && !$inputObj->IsOpenidSet()){//openid
		...
		//签名
		$inputObj->SetSign();//返回的都是大写字符串并且带有&符号

		$xml = $inputObj->ToXml();//转换成xml格式

		$startTimeStamp = self::getMillisecond();//请求开始时间
		$response = self::postXmlCurl($xml, $url, false, $timeOut);//将xml数据提交到curl接口，微信端只接受xml格式的数据
		$result = WxPayResults::Init($response);//将xml格式转化为array
		self::reportCostTime($url, $startTimeStamp, $result);//上报请求花费时间,及时上报返回的数据参数
	 ...
	 * @return 产生的随机字符串
	 */
	//很多老的 libc 的随机数发生器具有一些不确定和未知的特性而且很慢。PHP 的 rand() 函数默认使用 libc 随机数发生器。mt_rand() 函数是非正式用来替换它的，是rand函数的四倍。mt_rand(min,max)包括最小值和最大值
	...
	private static function reportCostTime($url, $startTimeStamp, $data)
	{
		...
		$objInput->SetInterface_url($url);//设置上报对应的接口的完整URL
	...
	{
		//获取毫秒的时间戳
		$time = explode ( " ", microtime () );//microtime函数获取的时间是有空格存在的，前面部分是毫秒，空格后面是自1970年到现在的秒数,microime(ture)则是返回浮点型的数据，是两个时间的和
		$time = $time[1] . ($time[0] * 1000);
		$time2 = explode( ".", $time );//防止毫秒数在0.000000089等这种情况的发生
		$time = $time2[0];
		return $time;
	}
}
