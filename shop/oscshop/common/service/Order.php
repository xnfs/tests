...
class Order{
	...
	public function cancel_order($order_id,$uid=null){
		...
		$order['order_status_id']=config('cancel_order_status_id');//将传过来的订单id的状态id修改后更新数据库，		
	...
	public function order_list($param=array(),$page_num=20,$uid=null){
		...
		$query=[];
		//视图view的参考资料https://www.kancloud.cn/manual/thinkphp5/156576
		//$map变量是Db后才使用到的一个变量
		if(isset($param['order_num'])){//判断订单号是否存在
			$map['Order.order_num_alias']=['eq',$param['order_num']];	
			$query['order_num']=urlencode($param['order_num']);
		}
		if(isset($param['username'])){//关键字的查询
		...
		->paginate($page_num,false,['query'=>$query]);//分页https://www.kancloud.cn/manual/thinkphp5/154294；默认生成的分页只有page一个参数，若需要提供额外的参数才能访问分页（例如查询结果的分页，需要传入查询关键字的参数才能显示结果），则需要设置额外参数query，相当于验证一下
	}
	...
	public function update_order($order_id){
		...
		$order['date_modified']=time();
		$order['pay_time']=time();
		Db::name('order')->update($order);//更新订单表
		...
		Db::name('order_history')->insert($order_history);//将订单记录写入订单记录表
		
		//更新积分
		if(!empty($order_info['return_points'])){//买东西付款后是可以获得积分的，还有就是用积分兑换一些商品或者券的扣积分			
			Db::name('points')->insert(//将新的积分写入积分表
				[
					'uid'=>$order_info['uid'],
					...
					'description'=>'下单积分',
					'prefix'=>1,//prefix：1是增加，2是减少
					'creat_time'=>time(),
					'type'=>1//用来区分是下单积分（1）还是管理员操作（2）
				]
			);
			Db::name('member')->where('uid',$order_info['uid'])->setInc('points',$order_info['return_points']);	//修改会员表里面的积分字段			
		}
		
	
		$member=Db::name('member')->where('uid',$order_info['uid'])->find();
		//存在上级代理商,本系统代理商只做一级分红
		if($member['pid']!=0){//等于0则意味是顶层代理商
			
			$agent_info=Db::name('agent')->where('uid',$member['pid'])->find();//agent：代理
