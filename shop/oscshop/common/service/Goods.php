...
class Goods{
     ...
     * object 对象实例
     */
    private static $instance;//此属性是存实例的，下次可以直接使用
	...
	public static function getInstance(){    
        if (!(self::$instance instanceof self))  //instenceof是运算符，判断某个引用是不是正真指向的类，如果是返回true;常量与静态属性和方法的访问是使用self；外部访问则是类名::方法名或者属性（参考http://www.jb51.net/article/74409.htm）
        {  
	...
	public function get_home_goods_list(){
		...
		if (!$home_goods_list= cache('home_goods_list')) {
			...
			$home_goods_list=Db::name('goods')->where('status',1)->order('goods_id desc')->limit(20)->select();//排序的方法为降序，因为新上的产品在后面，select方法返回的结果是一个二维数组
		
			cache('home_goods_list', $home_goods_list);//查询的数据存入缓存
			
		}
		// var_dump($home_goods_list);
	...
	public function get_goods_info($goods_id){
		...
		return [//重构数组
			...
			'discount'=>Db::name('goods_discount')->where('goods_id',$goods_id)->order('quantity ASC')->select(),//打折的商品信息
			'mobile_description'=>Db::name('goods_mobile_description_image')->where('goods_id',$goods_id)->order('sort_order asc')->select()//移动端的图片信息
		];
	}
	//更新商品点击量
	public function update_goods_viewed($goods_id){
		Db::execute("UPDATE ".config('database.prefix')."goods SET viewed = (viewed + 1) WHERE goods_id =".$goods_id);//与query相似，都是以原生态的sql语句写入数据。如果需要读取二级配置则可以：config('配置参数.二级参数');
	}
}
