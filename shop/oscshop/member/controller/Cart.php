...
class Cart extends HomeBase
{
    ...
    {		
		...
		$goods=$cart->get_all_goods(member('uid'));	//member('uid')获取会员信息	
    ...
    {    
		...
		if(!$uid=osc_service('member','user')->is_login()){//首先判断用户是否登录
			return ['error'=>'请先登录！！'];
		}
		
		$cart=osc_cart();//实例cart类
		
		$param=input('post.');//接受用户传递过来的所有post数据（type="hidden" value="{$goods.goods_id}"）
		...
		//验证商品数量和必选项
		if($error=$cart->check_quantity($param)){	//返回值都是错误信息，比如库存不足等		
			return $error;
		}	
		$param['uid']=$uid;
		//加入购物车	
