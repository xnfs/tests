...
class Transport extends AdminBase{
	...
	function index(){		
		...
		if ($list){
			...
			foreach ($list as $v) {
				if (!array_key_exists($v['id'],$transport)){
					$transport[$v['id']] = $v['title'];//支持快递公司的名字
				}
			}
			$extend = $model->getExtendList(array('transport_id'=>array('in',array_keys($transport))));
			// echo Db::table('transport_extend')->getLastSql();exit;查看上一次执行的sql语句
		...
		$data['extend']=$extend;//给原来的数组结构加两个键值，不会改变数组的维度
