...
class OrderMember extends MemberBase{
	...
	function index(){
		$this->assign('status',Db::name('order_status')->select());//订单状态表的数据
		$this->assign('list',osc_order()->order_list(input('param.'),20,member('uid')));//member是获取用户登录信息的一个函数,order_list是按条件查询订单
