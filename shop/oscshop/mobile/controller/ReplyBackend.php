...
class ReplyBackend extends AdminBase{
	...
	function text(){
		$this->assign('list',Db::name('wechat_text_reply')->paginate(config('page_num')));//分页输出，每页10条
	...
	function text_add(){
		...
		if(request()->isPost()){			
			...
			if(!$validate->check($data)){//返回的是bool值，true为验证通过				
			...
			$rid=Db::name('wechat_rule')->insert($rule,false,true);//写入数据库存储，返回成功添加的条数
	...
	function text_edit(){
		...
		if(request()->isPost()){			
			...
			if(!$validate->scene('edit')->check($data)){	//tp5的验证场景			
			    return ['error'=>$validate->getError()];				
			}
			...
			$r=Db::name('wechat_rule')->update($rule,false,true);//返回影响的数据条数
