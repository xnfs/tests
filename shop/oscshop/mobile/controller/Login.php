...
class Login extends MobileBase{
	...
	function refresh_member($user){
		...
		cookie('mobile_user_info',$user);//移动端会话信息直接存储在cookie里面		
	}
	...
	public function validate_login($data){
			...
			if(1==config('use_captcha')){//此配置是是否开启验证码的功能				
 	...
 	function login(){
		...
		if(request()->isPost()){
			...
			if(isset($r['error'])){
				...
				osc_service('mobile','user')->set_cart_total($r['total']);//将用户的商品数量存入session			
				return ['success'=>'登录成功','url'=>cookie('jump_url')];//获取存储在cookie里面的URL值，什么时候设置的？
			}
		}
