...
class Option extends AdminBase{
	...
	public function autocomplete(){
		...
		$model=osc_model('admin','option');//模型数据
		
		$options = $model->get_options($filter_name);//操作选项数据
		$json=[];
		foreach ($options as $option) {
			...
			if ($option['type'] == 'select' || $option['type'] == 'radio' || $option['type'] == 'checkbox') {
				...
				$option_values = osc_goods()->get_option_values($option['option_id']);	//二维数组		
				
				foreach ($option_values as $option_value) {
																		
					$option_value_data[] = array(
						'option_value_id' => $option_value['option_value_id'],
						'name'            => html_entity_decode($option_value['value'], ENT_QUOTES, 'UTF-8'),//函数把 HTML 实体转换为字符。第二参数为单双引号都转化为字符。htmlentities() 函数把字符转换为 HTML 实体。								
					);
				}
				...
				array_multisort($sort_order, SORT_ASC, $option_value_data);	//对多个数组或者多维数组进行排序,数组的维度应该一样吧
				/*
			...
			$json[] = array(
				...
				'name'         => strip_tags(html_entity_decode($option['name'], ENT_QUOTES, 'UTF-8')),//strip_tags去除字符串里面的html，xml，php标签
				'category'     => $type,
				'type'         => $option['type'],
				'option_value' => $option_value_data//操作选项对应值
			);
		}
