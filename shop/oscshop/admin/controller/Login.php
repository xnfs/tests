	...
	public function login(){
		...
		$user= osc_service('admin','user');
		 ...
		 if(request()->isPost()){
			...
			$user_info=Db::name('admin')->where('user_name',$data['username'])->find();//后台的管理员的数据表信息
		
			//用户存在且可用
			if($user_info&&$user_info['status']==1){//status状态值就是审核通过的账号
				...
				if(think_ucenter_encrypt($data['password'],config('PWD_KEY'))==$user_info['passwd']){
			        ...
			        $data = array();//$data数组是为了更新admin数据表的相关信息
            ...
            if($user->is_login()){//已经登录了
                $this->redirect('Index/index');//重定向到index
            }else{	//未登录的		
