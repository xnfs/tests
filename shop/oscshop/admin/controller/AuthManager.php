...
class AuthManager extends AdminBase{
	...
	public function access(){
	 ...
	 public function write_group(){
		...
		Db::name('auth_rule')->where(array('group_id'=>$group_id))->delete();//清空之前的权限
		...
		Db::name('auth_group')->update($data,false,true);//更新所属组表的信息
		
		$group_menu=Db::name('auth_group')->where('id',$group_id)->find();	//		

		$menu_id=explode(',', $group_menu['rules']);//权限代号是又菜单表的id值
